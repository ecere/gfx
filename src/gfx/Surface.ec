...
public:
   ...
   void Bevel(bool inner, int x, int y, int w, int h)
   {
      ...
      SetForeground(inner ? Color { 128,128,128 } : Color { 229, 234, 245 } /*formColor*/);
      HLine(x,   x+w - 2, y);
      VLine(y+1, y+h - 2, x);
      ...
      SetForeground(inner ? Color { 229, 234, 245 } /*formColor*/ : Color { 128,128,128 } );
      ...
      if(x2 >= box.left && x1 <= box.right && y2 >= box.top && y1 <= box.bottom)
      {
         ...
         for(c = start = firstPixel; c<=lastPixel; c++)
         {
            ...
            if(nextKey && nextKey->percent != key->percent)
            {
               ...
               newColor = { (byte)a, { (byte)r, (byte)g, (byte)b } };
            }
            else
               newColor = key ? key->color : 0;
