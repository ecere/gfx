...
// WPAL sets up its own that should be in sync
static int xSystemDepth;
static Visual * xSystemVisual;

// Set up here
static PixelFormat xSystemPixelFormat;
static bool xSharedMemory;

static X11Window hiddenPixmapWindow;

// REVIEW: pass through DisplaySystem?
static void * xGlobalDisplay;
...
class XDisplayDriver : DisplayDriver
{
   ...
   bool CreateDisplaySystem(DisplaySystem displaySystem)
   {
      ...
      xGlobalDisplay = displaySystem.xGlobalDisplay;
...

      {
         ...
         xSystemVisual = attributes.visual;
         {
            ...
            hiddenPixmapWindow = XCreateWindow(xGlobalDisplay, DefaultRootWindow(xGlobalDisplay),
               0,0,1,1,0, CopyFromParent, InputOnly, xSystemVisual, CWOverrideRedirect, &attributes);
         }
      }
   ...
   bool MakeDDBitmap(DisplaySystem displaySystem, Bitmap bitmap, bool mipMaps, int cubeMapFace)
   {
      ...
      xBitmap.pixmap =
         XCreatePixmap(xGlobalDisplay, hiddenPixmapWindow /*(X11Window)display.window*/, bitmap.width, bitmap.height,
      ...
      if(bitmap.transparent)
         xBitmap.mask =
            XCreatePixmap(xGlobalDisplay, hiddenPixmapWindow /*(X11Window)display.window*/, bitmap.width, bitmap.height, 1);
   ...
   bool GetSurface(Display display, Surface surface, int x,int y, Box clip)
   {
      ...
      if((surface.driverData = xSurface = XSurface { }))
      {
         ...
         xSurface.colorPixmap = XCreatePixmap(xGlobalDisplay, (X11Window)hiddenPixmapWindow /*display.window*/, 1, 1, /*(xSystemDepth == 16) ? 16 : */32);
   ...
   void BlitDI(Display display, Surface surface, Bitmap src, int dx, int dy, int sx, int sy, int w, int h)
   {
      ...
      else
      {
         ...
         else
         {
            ...
            pixmap = XCreatePixmap(xGlobalDisplay, (X11Window)hiddenPixmapWindow /*display.window*/, w, h, xSystemDepth /*24*/);
            mask = XCreatePixmap(xGlobalDisplay, (X11Window)hiddenPixmapWindow /*display.window*/, w, h, 1);
